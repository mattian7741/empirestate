...
| **Deployment** | **Disposable** hosts; **declarative** layers (intent → state tool e.g. Ansible → ops → **Docker** → libs); zero-to-op on fresh machine + credentials. |
...
No lock-in to one cloud; containers portable; state externalized; secrets outside host.
