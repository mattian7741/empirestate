...
**Scope:** Generally applicable strategy only (any application). App-specific strategy belongs in docs/DETAILED_DESIGN (see DEVELOPMENT_GUIDE § Where to capture strategy decisions).
...
- **Desktop apps (Electron).** For desktop applications that run inside Electron (and for similar GUI apps that will be distributed on macOS), the canonical approach is **build → publish to a stable URL → distribute via Homebrew Cask**.
  ...
  - **Distribute:** Provide a **Homebrew Cask** that points at that URL. Use either the public [homebrew-cask](https://github.com/Homebrew/homebrew-cask) repo (submit a cask for review) or an **organization tap** (see § Homebrew tap below).
  - **CI/CD:** Pipeline steps: build the Electron app → run tests → create release / upload artifact → (if using a tap) update cask `url` and `sha256` in the tap repo and push. The **hello-world MVP** is: run this pipeline once and install the app via `brew install --cask <cask>` (or run the built artifact) and verify it launches.
...
**Homebrew tap (organization)**

When using an organization tap (not the app repo as tap, not homebrew-cask):

- **Separate tap repo** — Use a dedicated `homebrew-tap` repo for all application casks. Do not put casks in the app repo; keep distribution metadata separate.
- **Repo naming** — Name the repo `homebrew-tap` so that `brew tap <user>/tap` works (Homebrew infers `homebrew-<suffix>` from the tap name).
- **Structure** — One Cask per app: `Casks/<app-name>.rb`. Example: `Casks/file-triage.rb`.
- **Install flow** — Users run `brew tap <user>/tap` then `brew install --cask <cask-name>`.
- **Cask URL and sha256** — Point the Cask at the release asset URL. Update `url` and `sha256` when releasing. **Release tag vs app version:** The GitHub release tag (e.g. `v0.1.2`) can differ from the app version in filenames (e.g. `App-0.1.0-arm64.dmg`) if `package.json` was not bumped before the release. Use the actual release tag in the URL path and the actual asset filename.
- **Computing sha256** — After a release, run `curl -sL -o /tmp/app.dmg "<release-url>"` then `shasum -a 256 /tmp/app.dmg` to get the correct sha256 for the Cask.
- **Gatekeeper (unsigned apps)** — Apps built without Apple code signing and notarization may show "damaged and can't be opened" on first launch. For local verification: `xattr -cr "/Applications/App Name.app"`. For production distribution, use code signing and notarization.
...
- **Comments rare and short.** Avoid comments, or keep them very brief. Use only to annotate confusing lines or a one-line description at the top of the method.
